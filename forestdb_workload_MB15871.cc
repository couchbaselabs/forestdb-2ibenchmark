...
void *do_incremental_writes(void *arg)
{
    ...
    *argcompact = 0;
    ...
    while(numincrmutationsdone<numincrmutations){
       ...
       getnextprimarykeylookup((char *)doc2->key, keylen);
...
void *do_reads(void *arg)
{
   ...
   assert(status == FDB_RESULT_SUCCESS);
   if(tid%2==0){
      status = fdb_kvs_open(fhandle, &kvhandle,"main" , &kvs_config);
      assert(status == FDB_RESULT_SUCCESS);
      printf("\nreader thread id: %d opened main", tid); 
   }
   else{
      status = fdb_kvs_open(fhandle, &kvhandle,"back" , &kvs_config);
      assert(status == FDB_RESULT_SUCCESS);
      printf("\nreader thread id: %d opened back", tid); 
   }   
